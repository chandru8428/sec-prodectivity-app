...
export function renderStep1(rawText) {
  return `<div class="glass-card fade-in">
    ...
    <div class="tabs mb-4">
      <button class="tab active" id="t-txt"><i data-lucide="clipboard-list" class="icon-inline"></i> Paste Text</button>
      <button class="tab" id="t-pdf"><i data-lucide="file-text" class="icon-inline"></i> Upload PDF</button>
    </div>
    <div id="a-txt">
    ...
    <div id="a-pdf" class="hidden">
      <label for="pdf-inp" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:160px;border:2px dashed var(--outline);border-radius:var(--radius-xl);cursor:pointer;gap:12px">
        <div style="font-size:48px;opacity:0.4"><i data-lucide="file-text" class="icon-inline"></i></div>
    ...
    <div class="flex justify-between mt-5">
      <button class="btn btn-ghost btn-sm" id="s1-reset" style="color:var(--color-danger);opacity:0.7"><i data-lucide="trash-2" class="icon-inline"></i> Reset All</button>
      <button class="btn btn-primary" id="s1b"><i data-lucide="zap" class="icon-inline"></i> Extract & Parse →</button>
    </div>
  </div>`;
}
...
export function renderStep2(subjects) {
  if (!subjects.length) {
    return `<div class="glass-card fade-in">
      ...
      <div class="text-muted" style="text-align:center;padding:40px">
        <div style="font-size:48px;margin-bottom:12px"><i data-lucide="alert-triangle" class="icon-inline"></i>️</div>No subjects found. Go back and check your input.
      </div>
  ...
  return `<div class="glass-card fade-in">
    ...
    <input id="subj-search" class="form-input mb-3" placeholder="<i data-lucide="search" class="icon-inline"></i> Search by name or code..." style="font-size:14px">
...
export function renderStep3(leaveDay, avoidSlots, selectedSubjects, staffPrefs, slotPrefs) {
  ...
  const subjectPickers = (selectedSubjects||[]).map((s, i) => {
    ...
    return `<div style="padding:14px;border-radius:var(--radius-lg);border:2px solid ${c}44;background:var(--bg-primary);margin-bottom:10px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label style="font-size:11px;font-weight:600;color:var(--color-on-surface-variant);display:block;margin-bottom:4px"><i data-lucide="user" class="icon-inline"></i> Preferred Staff</label>
          <select class="form-input staff-pref" data-key="${key}" style="font-size:13px;padding:8px 10px">
            ${staffOptions}
          </select>
        </div>
        <div>
          <label style="font-size:11px;font-weight:600;color:var(--color-on-surface-variant);display:block;margin-bottom:4px"><i data-lucide="ticket" class="icon-inline"></i> Specific Slot</label>
  ...
  return `<div class="glass-card fade-in">
    ...
    <h3 class="text-title mb-3"><i data-lucide="umbrella" class="icon-inline"></i> Leave Day</h3>
    ...
    <div class="flex justify-between mt-5">
      ...
      <button class="btn btn-primary" id="s3b"><i data-lucide="rocket" class="icon-inline"></i> Generate Timetable</button>
    </div>
  </div>`;
}
...
export function renderStep5(results, selectedSubjects, leaveDay, clashWarning, resultIndex = 0) {
  if (!results || !results.length) {
    return `<div class="glass-card fade-in">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-2 no-print" style="background:var(--bg-secondary);padding:10px 14px;border-radius:var(--radius-md);border:1px solid var(--border-color)">
        ...
        <button class="btn btn-ghost btn-sm" id="new-btn" style="color:var(--color-danger);font-weight:600"><i data-lucide="refresh-cw" class="icon-inline"></i> Start Over</button>
      </div>
      <h2 class="text-title-lg mb-4"><i data-lucide="x-circle" class="icon-inline"></i> No Valid Timetable Found</h2>
  ...
  const rows = DAYS.map(day => {
    ...
    return `<tr style="${isLeave?'opacity:0.3':''}">
      <td style="padding:8px 12px;font-weight:700;font-size:12px;white-space:nowrap;border:1px solid var(--border-color);background:var(--bg-secondary)">${day.slice(0,3)}${isLeave?' <i data-lucide="umbrella" class="icon-inline"></i>':''}</td>${cells}
    </tr>`;
  }).join('');
    ...
    return `<div style="margin-bottom:14px;border-radius:var(--radius-lg);border:1px solid var(--border-color);overflow:hidden">
      <div style="background:${c};padding:12px 16px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
        ...
        <div style="text-align:right">
          <div style="background:rgba(255,255,255,0.2);color:#fff;padding:4px 14px;border-radius:20px;font-weight:700;font-size:14px;font-family:monospace"><i data-lucide="ticket" class="icon-inline"></i> ${opt.slot_name}</div>
      ...
      <div style="padding:12px 16px;background:var(--bg-primary)">
        ${hasClasses ? scheduleRows : '<div style="color:var(--color-danger);font-size:12px;padding:4px 0"><i data-lucide="alert-triangle" class="icon-inline"></i>️ No class data. Please re-parse your timetable text.</div>'}
      </div>
    </div>`;
  }).join('');
  ...
  return `${printCss}<div class="glass-card fade-in" id="printable-area">
    ...
    <div class="flex justify-between items-center mb-4 flex-wrap gap-2 no-print">
      <h2 class="text-title-lg"><i data-lucide="check-circle-2" class="icon-inline"></i> Your Optimal Timetable</h2>
      <div class="flex gap-2 flex-wrap">
        ${results.length>1?`<select id="alt-select" class="btn btn-sm" style="width:auto;cursor:pointer;appearance:none;padding-left:16px;padding-right:32px;border:none;outline:none;background:linear-gradient(135deg, #6366f1, #a855f7, #ec4899);color:#fff;box-shadow:0 4px 15px rgba(168,85,247,0.4);border-radius:12px;font-weight:700;background-image:url('data:image/svg+xml;utf8,<svg fill=%22%23ffffff%22 height=%2224%22 viewBox=%220 0 24 24%22 width=%2224%22 xmlns=%22http://www.w3.org/2000/svg%22><path d=%22M7 10l5 5 5-5z%22/></svg>');background-repeat:no-repeat;background-position:right 4px center;transition:all 0.2s;" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 20px rgba(168,85,247,0.6)';" onmouseout="this.style.transform='none';this.style.boxShadow='0 4px 15px rgba(168,85,247,0.4)';">${results.map((r,i)=>`<option value="${i}" ${i === resultIndex ? 'selected' : ''} style="color:#333;background:#fff;font-weight:600;"><i data-lucide="sparkles" class="icon-inline"></i> Option ${i+1} — Score ${Math.min(100, Math.max(0, Math.round((r.score / (1000 + ((selectedSubjects||[]).length * 15) + 30)) * 100)))}%</option>`).join('')}</select>`:''}
        <button class="btn btn-secondary btn-sm no-print" id="regen-btn"><i data-lucide="refresh-cw" class="icon-inline"></i> Regenerate</button>
        <button class="btn btn-primary btn-sm no-print" id="print-btn"><i data-lucide="printer" class="icon-inline"></i> Print</button>
        <button class="btn btn-primary btn-sm" id="save-btn"><i data-lucide="save" class="icon-inline"></i> Save</button>
      </div>
    </div>
    ...
    ${!hasAnyData?`<div style="background:rgba(247,37,133,0.1);border:1px solid rgba(247,37,133,0.3);border-radius:var(--radius-lg);padding:14px 16px;margin-bottom:16px;font-size:13px">
      <i data-lucide="alert-triangle" class="icon-inline"></i>️ <strong>Grid is empty</strong> — class timing data not found. Please go back and re-parse your timetable text.
    </div>`:''}
    ...
    <div id="print-section">
      <h3 style="font-size:14px;font-weight:700;margin-bottom:10px"><i data-lucide="calendar" class="icon-inline"></i> Weekly Timetable</h3>
      ...
      <h3 style="font-size:14px;font-weight:700;margin-bottom:8px"><i data-lucide="clipboard-list" class="icon-inline"></i> Selected Slots</h3>
