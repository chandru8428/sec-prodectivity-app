...
export function render(root) {
  ...
  root.innerHTML = `
    <div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:var(--space-6);position:relative;z-index:1;">
      ...
      <div style="position:absolute;top:24px;right:24px;">
        <button id="register-theme-toggle" class="header-theme-toggle" aria-label="Toggle dark/light mode" style="background:var(--color-surface);border:1px solid var(--border-color);box-shadow:var(--shadow-sm);">
  ...
  // ── Theme Toggle Logic ──
  const themeToggle = root.querySelector('#register-theme-toggle');
  if (themeToggle) {
